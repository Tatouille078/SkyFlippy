...
function App() {
  ...
  const {slider1, slider2, slider3, slider4, setSlider1, setSlider2, setSlider3, setSlider4, handleSlide, lock1, lock2, lock3, lock4, setLock1, setLock2, setLock3, setLock4} = useStateContext();
  const params = [
    {
        name: 'Param1',
        slider: slider1,
            ...
            <div className="flex-1 flex justify-center px-2 md:px-4">
              <div className="relative w-full max-w-md">
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 text-sm md:text-base rounded-full bg-purple-700 text-white placeholder-purple-300 focus:outline-none focus:ring-2 focus:ring-white"
                />
      ...
      <div className="flex relative">
        ...
        >
          <div className="p-6 space-y-6">
            <h2 className="text-2xl font-bold text-purple-600 mb-4">Poids</h2>
            {params.map((param, index) => (
              <div key={index} className="space-y-2">
                <InputSlider props={param} handler={handleSlide} />
              </div>
            ))}
          </div>
        </div>
